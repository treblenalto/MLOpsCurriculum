...
def test_get_user_detail(api_client, test_user, user_1, user_2):
    ...
    assert data["id"] == test_user.id, "id is not correct"
    assert data["name"] == test_user.name, "name is not correct"
    assert data["age"] == test_user.age, "age is not correct"

# get a user with invalid id
def test_get_user_detail_invalid_id(api_client, test_user, user_1, user_2):
    response = api_client.get(reverse_lazy("user-detail", kwargs={"pk": -1}))
    assert response.status_code == status.HTTP_404_NOT_FOUND, "status code is not correct"
...
def test_post_user(api_client):
    ...
    response = api_client.post(reverse_lazy("user-list"), data=request_data, format="json")
...
def test_post_user_no_name(api_client):
    ...
    response = api_client.post(reverse_lazy("user-list"), data=request_data, format="json")
    assert response.status_code == status.HTTP_400_BAD_REQUEST, "status code is not correct"
    response_data = response.data
    assert isinstance(response_data, dict)

# create a user with no age
...
def test_update_user(api_client, test_user):
    ...
    response = api_client.put(reverse_lazy("user-detail", kwargs={"pk": test_user.id}), data=request_data, format="json")
    assert response.status_code == status.HTTP_200_OK, "status code is not correct"
...
def test_update_user_invalid_id(api_client, test_user):
    request_data = {
        "name": "UpdatedName",
        "age": 20
    }
    response = api_client.put(reverse_lazy("user-detail", kwargs={"pk": -1}), data=request_data, format="json")
    assert response.status_code == status.HTTP_404_NOT_FOUND, "status code is not correct"

# delete a user
def test_delete_user(api_client, test_user):
    response = api_client.delete(reverse_lazy("user-detail", kwargs={"pk": test_user.id}))
    assert response.status_code == status.HTTP_200_OK, "status code is not correct"
    response_data = response.data
...
def test_delete_user_invalid_id(api_client, test_user):
